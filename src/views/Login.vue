        ...
        onSubmit(user) {
            this.$refs[user].validate((valid) => {
                if (valid) {
                    ...
                    // 改为显式发送 urlencoded 字符串，并设置 Content-Type，增加错误日志
                    axios.post(baseUrl + '/oauth/token', params.toString(), {
                        auth,
                    ...
                    .catch(error => {
                        // 打印后端返回的详细信息，便于排查
                        console.error('oauth/token error response:', error.response && error.response.data)
                        this.$message.error((error.response && error.response.data && error.response.data.error_description) || '登录失败')
                    })
